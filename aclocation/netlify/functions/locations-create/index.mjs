...
export default handler(async (req) => {
    ...
    const [{ existing }] = (await db()
        .sql `SELECT EXISTS(SELECT 1 FROM locations WHERE slug = ${body.slug}) AS existing`);
    if (existing)
        throw new HttpError(409, 'slug_already_in_use');
  ...
  `;
    ...
    return json({ location: { ...location, netlifySite }, invite }, { status: 201 });
});
async function provisionNetlifySite(slug, locationId) {
    ...
    if (!res.ok)
        throw new Error(`netlify_site_create_${res.status}`);
