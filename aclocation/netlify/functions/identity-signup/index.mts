...
const handler: Handler = async (event: HandlerEvent) => {
  try {
    const payload = JSON.parse(event.body || '{}') as { user?: any }
    ...
    const [{ count }] = (await db()
      .sql`SELECT COUNT(*)::int AS count FROM location_members`) as { count: number }[]
    ...
    if (!inviteToken) {
      return {
        ...
        body: JSON.stringify({
          ...
          message:
            'Public signup is disabled. Ask the platform administrator for an invite.',
        }),
      }
    }
