...
type Body = {
  email: string
  role: 'member' | 'admin' | 'super_admin' | 'master_admin'
  locationId?: string
}
...
export default handler(async (req) => {
  ...
  if (!body.email || !body.role) throw new HttpError(422, 'email_and_role_required')

  const isMaster = actor.roles.includes('master_admin')
  const isSuper = actor.roles.includes('super_admin')

  if (!isMaster && !isSuper) throw new HttpError(403, 'forbidden')
  ...
  if (!isMaster) {
    ...
    locationId = locationId ?? (actor.appMetadata.default_location_id as string | null)
    if (!locationId) throw new HttpError(400, 'no_target_location')
    const member = await db()
      .sql`SELECT 1 FROM location_members WHERE location_id = ${locationId} AND user_id = ${actor.id} LIMIT 1`
    if (member.length === 0) throw new HttpError(403, 'not_a_member_of_target_location')
  }
