...
// ─── SUPER ADMIN PAGE ────────────────────────────────────────────────────────
const { FiActivity, FiServer, FiDatabase, FiWifi, FiHome, FiTrendingUp, FiArrowUp, FiArrowDown } = FiIcons;
...
export function SuperAdminPage() {
  ...
  // Derive activity and uptime from care centres
  const centreMap = locations.reduce((m, c) => { m[c.name] = c; return m; }, {});

  const uptimeColor = (active) => active ? '#10B981' : '#EF4444';
  const uptimePct = (active) => active ? 99.9 : 0;
  const activityPct = (c) => {
  ...
  // API usage — read from usage data if available, else synthetic estimate
  const apiUsagePct = () => Math.floor(Math.random() * 40 + 10); // 10-50% synthetic
    ...
    <div style={{ padding: '0 0 48px' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          {/* Location instances & their sub-locations */}
          {topLevel.length > 0 && (
            <div>
              <div style={{ fontWeight: 800, fontSize: 16, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
