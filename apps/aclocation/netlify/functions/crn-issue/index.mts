...
type Body = { requestId: string; clientId: string }

export default handler(async (req) => {
  const user = await requireRole(['admin', 'super_admin'])
  const locationId = await resolveTenant(req, user)
  const body = await readJson<Body>(req)
  if (!body.requestId || !body.clientId) throw new HttpError(400, 'request_and_client_required')
  ...
  try {
    ...
    const r = await client.query(
      `SELECT id, status FROM crn_requests WHERE id = $1 AND location_id = $2 FOR UPDATE`,
      [body.requestId, locationId],
    )
    if (r.rowCount === 0) throw new HttpError(404, 'request_not_found')
    if (r.rows[0].status !== 'pending') throw new HttpError(409, 'request_not_pending')

    const issued = await client.query(
      `INSERT INTO crns (location_id, client_id, request_id, number, issued_by)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, number, issued_at`,
      [locationId, body.clientId, body.requestId, number, user.id],
    )

    await client.query(
      `UPDATE crn_requests SET status = 'issued', resolved_at = NOW() WHERE id = $1`,
      [body.requestId],
    )
...
function generateCrnNumber(): string {
