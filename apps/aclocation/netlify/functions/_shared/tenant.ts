 ...
 */
export async function resolveTenant(req: Request, user: AuthedUser): Promise<string> {
  const headerId = req.headers.get('x-aclocation-id')
  const metaId = (user.appMetadata.default_location_id as string) || null
  const candidate = headerId || metaId
  if (!candidate) throw new HttpError(400, 'no_location_selected')

  // master_admin (platform sysadmin) and super_admin bypass membership.
  if (user.roles.includes('master_admin') || user.roles.includes('super_admin')) return candidate

  const rows = await db()
    .sql`SELECT 1 FROM location_members WHERE location_id = ${candidate} AND user_id = ${user.id} LIMIT 1`
  if (rows.length === 0) throw new HttpError(403, 'not_a_member_of_location')
  return candidate
}
