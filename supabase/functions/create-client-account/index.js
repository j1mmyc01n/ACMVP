...
Deno.serve(async (req) => {
    ...
    try {
        ...
        if (userError) {
            ...
            if (userError.message.includes('already been registered')) {
                const { data: existing } = await supabaseAdmin
                    ...
                    .eq('email', email)
                    .single();
                return new Response(JSON.stringify({ success: true, existing: true, account: existing }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
            }
            throw userError;
        }
        ...
        if (magicError)
            console.error('Magic link error (non-fatal):', magicError);
        return new Response(JSON.stringify({ success: true, account }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    catch (error) {
