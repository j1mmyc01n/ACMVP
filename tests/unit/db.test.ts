...
describe('clients_1777020684735', () => {
  ...
  it('error path — propagates error object', async () => {
    ...
    const from     = vi.fn().mockReturnValue(chain);
    const result   = await (from('clients_1777020684735').then as unknown as () => Promise<unknown>)();
    expect((result as { error: { message: string } }).error.message).toBe('DB error');
  });
});
...
describe('profile_audit_log', () => {
  ...
  it('error path — returns error object', async () => {
    ...
    const chain    = makeChain(resolved);
    const from     = vi.fn().mockReturnValue(chain);
    const result   = await (from('profile_audit_log').insert({}).then as unknown as () => Promise<unknown>)();
