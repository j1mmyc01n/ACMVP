...
describe('FeedbackModal — feedback_tickets_1777090000', () => {
  ...
  it('insert: error path — error object is returned', async () => {
    ...
    const from     = vi.fn().mockReturnValue(chain);
    const result   = await (from('feedback_tickets_1777090000').insert([{}]).then as unknown as () => Promise<unknown>)();
...
describe('OTP Login — login_otp_codes_1777090007', () => {
  ...
  it('insert: error — bad email returns error', async () => {
    ...
    const chain    = makeChain(resolved);
    const from     = vi.fn().mockReturnValue(chain);
    const result   = await (from('login_otp_codes_1777090007').insert([{ email: '' }]).then as unknown as () => Promise<unknown>)();
    expect((result as { error: { message: string } }).error).toBeTruthy();
  });
});
...
describe('Client Registration — clients_1777020684735', () => {
  ...
  it('insert: error — duplicate CRN returns error', async () => {
    ...
    const chain    = makeChain(resolved);
    const from     = vi.fn().mockReturnValue(chain);
    const result   = await (from('clients_1777020684735').insert([{ full_name: 'Dupe', crn: 'CRN-001' }]).then as unknown as () => Promise<unknown>)();
    expect((result as { error: { code: string } }).error?.code).toBe('23505');
  });
...
describe('Care Centre Form — care_centres_1777090000', () => {
  ...
  it('insert: error — missing name returns error', async () => {
    ...
    const chain    = makeChain(resolved);
    const from     = vi.fn().mockReturnValue(chain);
    const result   = await (from('care_centres_1777090000').insert([{}]).then as unknown as () => Promise<unknown>)();
    expect((result as { error: { message: string } }).error).toBeTruthy();
  });
...
describe('Admin User Form — admin_users_1777025000000', () => {
  ...
  it('update: error — constraint violation', async () => {
    ...
    const chain    = makeChain(resolved);
    const from     = vi.fn().mockReturnValue(chain);
    const result   = await (from('admin_users_1777025000000').update({ role: 'invalid_role' }).eq('id', 'a1').then as unknown as () => Promise<unknown>)();
    expect((result as { error: { message: string } }).error).toBeTruthy();
  });
});
...
describe('Push Notifications Form — push_notifications_1777090000', () => {
  ...
  it('insert: error — missing title', async () => {
    ...
    const chain    = makeChain(resolved);
    const from     = vi.fn().mockReturnValue(chain);
    const result   = await (from('push_notifications_1777090000').insert([{}]).then as unknown as () => Promise<unknown>)();
